...
export default function WelcomePage() {
  const { scrollY } = useScroll(); //scrollY : the number of pixels the user scrolled

  //transform pixel values to opacity values with useTrznsform hook
  const opacityCity = useTransform(scrollY, [0, 200, 300, 500], [1, 0.5, 0.5, 0]) //the user scrolled from 0 at the top to 200px down / the opacity at the biginning 1 and then 50%
  ...
  return (
    <>
      <header id="welcome-header">
        ...
        <motion.img
          style={{ opacity: opacityCity, y: yCity }} // it will apply the effect without re-rendering the component
          animate={{ opacity: 1 }}
          src={cityImg}
