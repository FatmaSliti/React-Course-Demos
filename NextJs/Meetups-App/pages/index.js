...
import { MongoClient } from "mongodb"; //Next will detect that it's only used in the server side code so it won't show it to the user
...
//N.B: getStaticProps() & getServerSideProps() came to solve this problem:
//if we use useEffect and useSatate to fetch the data and display it , when the page is first rendered the useState has an empty array and doesn't contain the data
//so the pre-generated page will not contain the data which has worse initial page load performance and is less SEO-friendly
...
export async function getStaticProps() {
    ...
    //5-all the code inside of will never end up on the client side(we can securely connect to a DB)
    ...
    return {
        props: { //it has to be named props and its content is passed via props to the component
            ...
            // meetups: meetups, //problem with the strange id generated by mongo so we need to tweak the code =>
...
//which one is better?
//The getStaticProps is faster (because the data will be cached and reused) instead pre-generating and fetching the data for every incoming request
